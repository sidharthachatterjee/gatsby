...
async function runQuery(args) {
  if (args.nodeTypeNames.length > 1) {
    const nodes = args.nodeTypeNames.reduce(async (acc, typeName) => {
      acc.push(...(await getNodesByType(typeName)))
      return acc
    }, [])
  ...
  // console.log({ lokiArgs, firstOnly })
  ...
  let result
  if (firstOnly) {
    result = await findNode(query)
  } else {
    result = await findNodes(query)
  }
  // console.log({ result })
  return result

  // find(lokiArgs, firstOnly)
